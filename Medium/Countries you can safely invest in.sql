...
from (select *, SUBSTRING(phone_number,0,4) as country_code 
from person ) p
left join Country c
...
caller_country_duration as 
(
select caller_id,country,duration
from Calls c 
left join caller_country cc
on c.caller_id = cc.id

union ALL 

select callee_id as caller_id,country,duration
from Calls c 
left join caller_country cc
